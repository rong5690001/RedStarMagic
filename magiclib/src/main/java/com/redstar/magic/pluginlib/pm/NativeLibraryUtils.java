...
final class NativeLibraryUtils {
    ...
    private static final String TAG = NativeLibraryUtils.class.getSimpleName();
    ...
    static void copyNativeBinaries(@NonNull File apkFile, @NonNull File sharedLibraryDir) throws CopyNativeSoException {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                for (String abi : Build.SUPPORTED_ABIS) {
                    if (copyNativeBinaries(zipFile, sharedLibraryDir, abi)) {
                        Logger.v("copyNativeBinaries ok, abi: %s", abi);
                        return;
                    }
                }
            }
            ...
            if (copyNativeBinaries(zipFile, sharedLibraryDir, Build.CPU_ABI)) {
                Logger.v("copyNativeBinaries ok, abi: %s", Build.CPU_ABI);
                return;
            }

            if (copyNativeBinaries(zipFile, sharedLibraryDir, Build.CPU_ABI2)) {
                Logger.v("copyNativeBinaries ok, abi: %s", Build.CPU_ABI2);
                return;
            }

            if (copyNativeBinaries(zipFile, sharedLibraryDir, ABI_ARMEABI)) {
                Logger.v("copyNativeBinaries ok, abi: %s", ABI_ARMEABI);
                return;
            }

            Logger.v("copyNativeBinaries not found");
     ...
     * @see #ABI_MIPS64
     */
    static boolean copyNativeBinaries(@NonNull ZipFile zipFile, @NonNull File sharedLibraryDir,
                                      @NonNull String abi)
